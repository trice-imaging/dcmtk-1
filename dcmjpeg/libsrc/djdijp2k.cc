...
OFCondition DJDecompressJP2k::decode( Uint8 *compressedFrameBuffer, Uint32 compressedFrameBufferSize, Uint8 *uncompressedFrameBuffer, Uint32 uncompressedFrameBufferSize, OFBool isSigned)
{
	OPJSupport* supp = new OPJSupport();
	int colorModel;
	long uncompressedSize = 0;
        int planarConfig = cparam->getPlanarConfiguration();
	void* res = supp->decompressJPEG2KWithBuffer(uncompressedFrameBuffer, compressedFrameBuffer, compressedFrameBufferSize,&uncompressedSize, planarConfig, &colorModel);
	if (!res)
