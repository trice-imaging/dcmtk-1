  ...
  Sint32 opt_instanceNumber)
{
    ...
    if (result.good()) result = dataset->putAndInsertString(DCM_SpecificCharacterSet, "ISO_IR 192");
...
#ifdef TRICE
static void addTriceAttributes(const char *opt_seriesFile, DcmDataset* meta)
{   DcmFileFormat dfile;
    ...
    else
    {
      ...
      {   const char *c = NULL;
          ...
          c = NULL; if (dset->findAndGetString(DCM_StudyDescription, c).good() && c) 
                meta-> putAndInsertString(DCM_StudyDescription, c);
          c = NULL; if (dset->findAndGetString(DCM_PatientName, c).good() && c) 
                meta-> putAndInsertString(DCM_PatientName  , c);
          c = NULL; if (dset->findAndGetString(DCM_PatientID, c).good() && c) 
                meta-> putAndInsertString(DCM_PatientID, c);
          c = NULL; if (dset->findAndGetString(DCM_PatientSex, c).good() && c) 
                meta-> putAndInsertString(DCM_PatientSex, c);
          c = NULL; if (dset->findAndGetString(DCM_PatientBirthDate, c).good() && c) 
                meta-> putAndInsertString(DCM_PatientBirthDate, c);
          c = NULL; if (dset->findAndGetString(DCM_PatientAddress, c).good() && c) 
                meta-> putAndInsertString(DCM_PatientAddress, c);
          c = NULL; if (dset->findAndGetString(DCM_PerformingPhysicianName, c).good() && c) 
                meta-> putAndInsertString(DCM_PerformingPhysicianName , c);
          c = NULL; if (dset->findAndGetString(DCM_ReferringPhysicianName, c).good() && c) 
                meta-> putAndInsertString(DCM_ReferringPhysicianName, c);
          c = NULL; if (dset->findAndGetString(DCM_ReferringPhysicianAddress, c).good() && c) 
          ...
          c = NULL; if (dset->findAndGetString(DCM_PatientTelephoneNumbers, c).good() && c) 
                meta-> putAndInsertString(DCM_PatientTelephoneNumbers, c);
          c = NULL; if (dset->findAndGetString(DCM_ReferringPhysicianTelephoneNumbers, c).good() && c) 
                meta-> putAndInsertString(DCM_ReferringPhysicianTelephoneNumbers, c);  
          c = NULL; if (dset->findAndGetString(DCM_AdmittingDiagnosesDescription, c).good() && c) 
                meta-> putAndInsertString(DCM_AdmittingDiagnosesDescription, c);  
          bool privateTag = false;
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1011, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1011, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1012, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1012, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1022, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1022, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1010, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1010, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1013, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1013, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1023, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1023, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1024, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1024, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          c = NULL; if (dset->findAndGetString(DcmTag(0x6301, 0x1025, EVR_LO), c).good() && c) 
          {      DcmTag tag(0x6301, 0x1025, EVR_LO);    tag.setPrivateCreator("KRETZ_PRIVATE");
                 meta-> putAndInsertString( tag, c);  privateTag = true;  
          }
          if (privateTag)
                meta-> putAndInsertString( DcmTag(0x6301, 0x0010, EVR_LO) , "KRETZ_PRIVATE");
      }
   }
}
...
//**Read in the JSON, build internal dataset, write to a temp area (scratch), and treat like --study-from
      cmd.addOption("--help", "-?", "pdf2dcm .  This is the command that will convert any .pdf file to an encapsulated DCM for download.  It takes Trice .JSON file (for meta) or an optional DICOM file that contains the meta. \n \
 \n \
 * pdf2dcm --meta-xml <JSON file> [ or --study-from <DICOM file w meta> (or --meta-json <file> ]  inputFile  outputFile \n \
    * Return info \n \
...
//**TRICE : add in the attributes that came in through the meta-json
    if (cmd.findOption("--meta-json"))
    {     OFString metaFile;
          app.checkValue(cmd.getValue(metaFile));
          if (!fileExists((char*)metaFile.c_str()))
          {   OFLOG_FATAL(pdf2dcmLogger, "Meta JSON file does not exist");
              if (!isnull(dcmInputFileName.c_str()))  rmFile(dcmInputFileName.c_str());
              return 11;
          }
          char* json = readTxt((char*)metaFile.c_str());
          populateFromJson(json, fileformat.getDataset());
    }
#ifndef __windows__
    else if (cmd.findOption("--meta-xml"))
    {     OFString xmlFile;
          app.checkValue(cmd.getValue(xmlFile));
          if (!fileExists((char*)xmlFile.c_str()))
          {   OFLOG_FATAL(pdf2dcmLogger, "Meta XML file does not exist");
              if (!isnull(dcmInputFileName.c_str()))  rmFile(dcmInputFileName.c_str());
              return 11;
          }

          dcmInputFileName = xmlFile;  dcmInputFileName.append(".T.dcm");

          char* xml = ::readTxt((char*)xmlFile.c_str());
          bool xchange = false;
          ...
          {   xml = Util::replace(xml, "<file-format>", "<file-format><meta-header xfer=\"1.2.840.10008.1.2.1\" name=\"Little Endian Explicit\">");
              ...
              xchange = true;
              xmlFile.append(".1.xml");
              writeTxt((char*)xmlFile.c_str(), "", xml);
          }

                //**Save File -- linux only
          OFString command("xml2dcm -q --update-meta-info ");  command.append(xmlFile).append(" ").append(dcmInputFileName.c_str());
          if (system(command.c_str()) != 0)
          {    OFLOG_FATAL(pdf2dcmLogger, "system call for xml2dcm with input file failed");
               if (!isnull(dcmInputFileName.c_str()))  rmFile(dcmInputFileName.c_str());
               if (xchange)  rmFile(xmlFile.c_str());
               return 12;
          }
          if (xchange)  rmFile(xmlFile.c_str());

          createIdentifiers(opt_readSeriesInfo, dcmInputFileName.c_str(), studyUID, seriesUID, patientName, patientID, patientBirthDate, patientSex,incrementedInstance);
          result = insertPDFFile(fileformat.getDataset(), opt_ifname);
          result = createHeader(fileformat.getDataset(), patientName.c_str(), patientID.c_str(), patientBirthDate.c_str(), patientSex.c_str(), opt_annotation, studyUID.c_str(), seriesUID.c_str(), opt_documentTitle, opt_conceptCSD, opt_conceptCV, opt_conceptCM, OFstatic_cast(Sint32, opt_instance));
          addTriceAttributes(dcmInputFileName.c_str(), fileformat.getDataset());
    }
#endif
