...
OFCondition DcmSpecificCharacterSet::selectCharacterSetWithoutCodeExtensions()
{
    ...
    OFString fromEncoding;
         //**Be more tolerant of CharacterSets being passed in
#ifdef TRICE
    OFString _SourceCharacterSet = Util::replace(Util::replace(Util::replace((char*)SourceCharacterSet.c_str(),"_2022", ""), "IR_", "IR "), "ISO 2022 IR ", "ISO_IR ");
#else
    OFString _SourceChacacterSet = SourceCharacterSet;
#endif
    if (_SourceCharacterSet == "ISO_IR 6")           // ASCII
    {
        DCMDATA_WARN("DcmSpecificCharacterSet: 'ISO_IR 6' is not a defined term in DICOM, "
            << "will be treated as an empty value (ASCII)");
    ...
    else if (_SourceCharacterSet == "ISO_IR 100")    // Latin alphabet No. 1
        fromEncoding = "ISO-8859-1";
    else if (_SourceCharacterSet == "ISO_IR 101")    // Latin alphabet No. 2
        fromEncoding = "ISO-8859-2";
    else if (_SourceCharacterSet == "ISO_IR 109")    // Latin alphabet No. 3
        fromEncoding = "ISO-8859-3";
    else if (_SourceCharacterSet == "ISO_IR 110")    // Latin alphabet No. 4
        fromEncoding = "ISO-8859-4";
    else if (_SourceCharacterSet == "ISO_IR 144")    // Cyrillic
        fromEncoding = "ISO-8859-5";
    else if (_SourceCharacterSet == "ISO_IR 127")    // Arabic
        fromEncoding = "ISO-8859-6";
    else if (_SourceCharacterSet == "ISO_IR 126")    // Greek
        fromEncoding = "ISO-8859-7";
    else if (_SourceCharacterSet == "ISO_IR 138")    // Hebrew
        fromEncoding = "ISO-8859-8";
    else if (_SourceCharacterSet == "ISO_IR 148")    // Latin alphabet No. 5
        fromEncoding = "ISO-8859-9";
    else if (_SourceCharacterSet == "ISO_IR 13")     // Japanese
        fromEncoding = "JIS_X0201";                 // - "ISO-IR-13" is not supported by libiconv
    else if (_SourceCharacterSet == "ISO_IR 166")    // Thai
        fromEncoding = "ISO-IR-166";
    else if (_SourceCharacterSet == "ISO_IR 192")    // Unicode in UTF-8 (multi-byte)
        fromEncoding = "UTF-8";
    else if (_SourceCharacterSet == "GB18030")       // Chinese (multi-byte)
        fromEncoding = "GB18030";
    else if (_SourceCharacterSet == "GBK")           // Chinese (multi-byte, subset of GB18030)
        fromEncoding = "GBK";
    else {
        // create an appropriate error code
        OFOStringStream stream;
...
OFCondition DcmSpecificCharacterSet::selectCharacterSetWithCodeExtensions(const unsigned long sourceVM)
{
    ...
    while ((i < sourceVM) && status.good())
    {
...
#ifdef TRICE  //**Be more tolerant of names passed
        else
            definedTerm = Util::replace((char*)definedTerm.c_str(), "_", " ");
#endif
        ...
        else if (definedTerm == "ISO 2022 IR 58")       // Simplified Chinese (multi-byte)
        {
            ...
            status = makeOFCondition(OFM_dcmdata, EC_CODE_CannotSelectCharacterSet, OF_error, message.c_str());
#ifdef TRICE
            definedTerm = "ISO 2022 IR 6";             // ASCII
            encodingName = "ASCII";
            status = EC_Normal;
#endif
        }
        ...
        while ((pos < fromLength) && status.good())
        {
            ...
            if (isEscape || isDelimiter)
            {
                ...
                if (convertLength > 0)
                {
                    ...
                    DCMDATA_DEBUG("    Converting sub-string '"
                    ...
                    if (status.bad())
                        DCMDATA_DEBUG("    -> ERROR: " << status.text());
                }
