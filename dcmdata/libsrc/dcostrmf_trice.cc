...
#ifdef TRICE
  bool inTrice = BoxStore::inTrice();
  if (inTrice)
  {   const int BlockSize = 1024*1024;
      int sent = 0, left = buflen, t;
      while (left > 0)
      {    if (left > BlockSize)
           {   t = BoxStore::fwrite((char*)buf+sent, 1, BlockSize, NULL);  
               if (t != BlockSize)  return t+sent;
           }
           else
           {   t = BoxStore::fwrite((char*)buf+sent, 1, left, NULL);  
               if (t != left)  return t+sent;
           }
           sent += t;  left -= t;
      }
      return sent;
