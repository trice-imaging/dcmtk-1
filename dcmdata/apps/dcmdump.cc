...
#ifdef TRICE
static const OFString *printTagCreators[MAX_PRINT_TAG_NAMES];
#endif
...
static OFBool addPrintTagName(const char *tagName)
{
    ...
    if (sscanf(tagName, "%x,%x", &group, &elem) != 2)
    {
        ...
        if (dicent == NULL) {
            ...
            printTagCreators[printTagCount] = NULL;
#ifdef TRICE
                //**parallel arrays for private creator identifers
            const char* cc = dicent-> getPrivateCreator();
            printTagCreators[printTagCount] = (cc==NULL)?NULL:new OFString(cc);
#endif
        }
        dcmDataDict.unlock();
    } else {
    ...
    if (printTagCount == 0)
    {
        ...
        for (int i = 0; i < printTagCount; i++)
        {
            ...
            if (printTagKeys[i])
              ...
              //**resolve locatable private keys
#ifdef TRICE

            group = searchKey.getGroup();
            elem = searchKey.getElement();
            if (group%2 == 1 && elem <= 0x00ff && printTagCreators[i])  //**private key, relocatable
            {
                 const OFString* creator = printTagCreators[i];
                 const u_int16_t upper = findSpecificPrivateSpace((DcmDataset*)dset, group, (char*)creator->c_str(), 0x10);
                 searchKey.setElement(createLowerShort(upper, elem));
            }
#endif
            DcmStack stack;
